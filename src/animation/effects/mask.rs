...
/// Returns (center, half_size, rotation, blend_params) where
...
pub fn update_unified_mask_system(
    ...
    for (mask_info, material_handle, _marker, entity_global_transform) in query.iter() {
        ...
        let (mask1_center, mask1_half_size, mask1_rotation, mask1_blend) = compute_mask_params(
            mask1,
            pending,
            &mask_layer_query,
        ...
        if active_masks.len() >= 2 {
            ...
            let (mask2_center, mask2_half_size, mask2_rotation, mask2_blend) = compute_mask_params(
                mask2,
                pending,
                &mask_layer_query,
                playback.current_time_ms,
