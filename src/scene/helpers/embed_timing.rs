...
pub(crate) fn build_embed_scene_timing_plan(
    ...
    // When outTime is present, derive the effective inner speed from the
    // time-mapping ratio instead of the declared `speed` attribute.
    // AM maps parent [startTime..endTime] → nested [inTime..outTime], so
    // effective_inner = (outTime − inTime) / (endTime − startTime).
    // This correctly handles negative outTime (backwards playback).
